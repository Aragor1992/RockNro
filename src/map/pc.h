...
BEGIN_ZEROED_BLOCK; // all the variables within this block get zero'ed in each call of status_calc_pc
	...
	int addsize[3];
	//short hp_ratio_atk_race[RC_MAX]; //Ataque en base al HP del enemigo. [Yussel] (Rock & RO)
//	int dmg_per_hp_adjust[RC_MAX]; // Ataque por HP [Yussel] Rock & RO. 14/11/2014
//	int dmg_per_hp_ratio[RC_MAX]; // Ataque por HP [Yussel] Rock & RO. 14/11/2014
	struct drain_data {
		int rate;
		int per;
		int value;
...
BEGIN_ZEROED_BLOCK; // this block will be globally zeroed at the beginning of status_calc_pc()
	...
	int mdmg_per_hp_adjust[RC_MAX];//Ataque por HP magico [Yussel] Rock & RO. 14/11/2014
	int mdmg_per_hp_ratio[RC_MAX];//Ataque por HP magico [Yussel] Rock & RO. 14/11/2014
#ifdef RENEWAL
	...
	struct {
		...
		unsigned int dispell_resist; // Dispell resist [Yussel] Rock & RO. 28/05/2015
	} bonus;
END_ZEROED_BLOCK;
	...
	int magic_flee; // Flee magico [Yussel] Rock & RO. 14/11/2014
	int itemid;
