			...
			switch(skill_id){
				...
				case AL_HOLYLIGHT:
					...
					//if (sc && sc->data[SC_SOULLINK] && sc->data[SC_SOULLINK]->val2 == SL_PRIEST)
						//skillratio *= 5; //Does 5x damage include bonuses from other skills? ORIGINAL Desactivado por [Yussel] 09/06/2015.
					break;
				case AL_RUWACH:
					skillratio += 45;
					break;
				...
				case SL_STUN:
					...
					break;
				case SL_SMA:
					skillratio += -40 + status->get_lv(src); //Base damage is 60% + lv%
					break;
				case NJ_KOUENKA:
					skillratio -= 10;
...
static struct Damage battle_calc_magic_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int mflag)
{
	int nk;
	short s_ele = 0;
	struct map_session_data *sd, *tsd = NULL;
	...
	// Flee magico [Yussel] Rock & RO. 14/11/2014
	if(tsd && target) {
		if(rand()%10000 < tsd->magic_flee){
			//clif_specialeffect(target, 500, AREA);
			clif->specialeffect(target,500,AREA);
			ad.damage = 0;
			return ad;
		}
	}
...
/*Yussel
#define mdmg_per_hp_ratio( a ) ( ad.damage+= ad.damage*(a)/100 )
Yussel*/
...
/**/
			//Ataque magico en base al HP del enemigo. [Yussel] (Rock & RO) 
		if(sd){
			// Agregado [Yussel] 14/10/2014.
			
			// mdmg_per_hp_adjust:  A partir de dmg_per_hp_adjust HP empieza a funcionar el bonus.
			int adjusted_hp_race = tstatus->hp - sd->mdmg_per_hp_adjust[tstatus->race];
			int adjusted_hp_boss = tstatus->hp - sd->mdmg_per_hp_adjust[is_boss(target)?RC_BOSS:RC_NONBOSS];
			
			int adjusted_atk_race, adjusted_atk_boss;
			
			if(adjusted_hp_race < 0){
				adjusted_hp_race = 0;
			}
			
			if(adjusted_hp_boss < 0){
				adjusted_hp_boss = 0;
			}
			
			// mdmg_per_hp_ratio: Cada mdmg_per_hp_ratio de HP aumenta 1% el daño de ataque.
			adjusted_atk_race = adjusted_hp_race / sd->mdmg_per_hp_ratio[tstatus->race];
			adjusted_atk_boss = adjusted_hp_boss / sd->mdmg_per_hp_ratio[is_boss(target)?RC_BOSS:RC_NONBOSS];
			
			// Cap atk% at double.
			if(adjusted_atk_race > 100){
				adjusted_atk_race = 100;
			}
			if(adjusted_atk_boss > 100){
				adjusted_atk_boss = 100;
			}
			ad.damage += ad.damage*adjusted_hp_race? adjusted_atk_race:0/100;
			ad.damage += ad.damage*adjusted_hp_race? adjusted_atk_boss:0/100;
//			ad.damage=adjusted_hp_race? adjusted_atk_race:0;
//			ad.damage=adjusted_hp_race? adjusted_atk_boss:0;
		}
/**/
		//Ataque magico en base al HP del enemigo. [Yussel] (Rock & RO)
...
/*
		//Ataque en base al HP del enemigo. [Yussel] (Rock & RO) 
		if(skill_id != MO_INVESTIGATE && skill_id != MO_EXTREMITYFIST){
			// Modificado [Yussel] 14/10/2014.
			
			// dmg_per_hp_adjust:  A partir de dmg_per_hp_adjust HP empieza a funcionar el bonus.
			int adjusted_hp_race = tstatus->hp - sd->right_weapon.dmg_per_hp_adjust[tstatus->race];
			int adjusted_hp_boss = tstatus->hp - sd->right_weapon.dmg_per_hp_adjust[is_boss(target)?RC_BOSS:RC_NONBOSS];
			...
			// dmg_per_hp_ratio: Cada dmg_per_hp_ratio de HP aumenta 1% el daño de ataque.
			adjusted_atk_race = adjusted_hp_race / sd->right_weapon.dmg_per_hp_ratio[tstatus->race];
			adjusted_atk_boss = adjusted_hp_boss / sd->right_weapon.dmg_per_hp_ratio[is_boss(target)?RC_BOSS:RC_NONBOSS];
			...
			ATK_ADDRATE(adjusted_atk_race);
			ATK_ADDRATE(adjusted_atk_boss);
		}
*/
		//Card Fix, sd side
	...
	if( sc ) {
		...
		if (sc->data[SC_FUSION]) {
			...
			// hp = 2*hp/100; //2% hp loss per hit
				// status_zap(src, hp, 0);
			//Yussel Rock And Ro
			if ((sstatus->hp * 100) <= (sstatus->max_hp * 30))
			status_zap(src, hp, 0);
		}
...
static enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, int64 tick, int flag)
{
	...
	//Rock and RO [Yussel]
	if(sd && (skillv = pc->checkskill(sd,CH_CHAINCRUSH)) > 0) {
		int chain_rate= 14 - skillv; //Base Rate
		if (sc && sc->data[SC_SKILLRATE_UP] && sc->data[SC_SKILLRATE_UP]->val1 == CH_CHAINCRUSH) {
			chain_rate+= chain_rate*(sc->data[SC_SKILLRATE_UP]->val2)/100;
			status_change_end(src, SC_SKILLRATE_UP, INVALID_TIMER);
		}
		if (rnd()%100 < chain_rate) {
			if( skill->attack(BF_WEAPON,src,src,target,CH_CHAINCRUSH,skillv,tick,0) )
				return ATK_DEF;
			return ATK_MISS;
		}
	}//termina Rock and RO [Yussel]
