...
static int pc_bonus3(struct map_session_data *sd, int type, int type2, int type3, int val)
{
	...
	switch(type){
/*		case SP_HP_RATIO_ATK_RACE: // Ataque por HP [Yussel] Rock & RO. 14/11/2014
			{
			uint32 race_mask = map->race_id2mask(type2);
			if (race_mask == RCMASK_NONE) {
				ShowWarning("pc_bonus3: SP_HP_RATIO_ATK_RACE: Invalid Race (%d)\n", type2);
				break;
			}
		sd->right_weapon.dmg_per_hp_adjust[type2]=cap_value(type3, 0, INT_MAX);
		sd->right_weapon.dmg_per_hp_ratio[type2]=cap_value(val, 1, INT_MAX);
		}
		break;*/
/**/		case SP_HP_RATIO_MATK_RACE:
			{
			uint32 race_mask = map->race_id2mask(type2);
			if (race_mask == RCMASK_NONE) {
				ShowWarning("pc_bonus3: SP_HP_RATIO_MATK_RACE: Invalid Race (%d)\n", type2);
				break;
			}
			sd->mdmg_per_hp_adjust[type2]=cap_value(type3, 0, INT_MAX);
			sd->mdmg_per_hp_ratio[type2]=cap_value(val, 1, INT_MAX);
			}
			break;/**/
		case SP_ADD_MONSTER_DROP_ITEM:
		{
...
static int64 pc_readparam(const struct map_session_data *sd, int type)
{
	...
	switch(type) {
		...
		case SP_MAGIC_FLEE:      val = sd->magic_flee; break;
		...
		case SP_DISPELL_RESIST:  val = sd->bonus.dispell_resist; break;
#ifdef RENEWAL_CAST
